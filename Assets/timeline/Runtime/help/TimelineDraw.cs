...
public class TimelineDraw
{
    ...
    private static GameObject sharedObj;
    private static MeshFilter sharedFilter;
    private static MeshRenderer sharedRender;
    private static Material sharedMat;


    private static bool SetupIfNoExist()
    {
        if (sharedObj == null)
        {
    ...
    /// 绘制实心圆形
    /// </summary>
    public static GameObject DrawCircleSolid(Transform t, float radius)
    {
    ...
    public static GameObject DrawCircleSolid(Vector3 center, Vector3 forward, float radius)
    {
        int pointAmount = 100;
    ...
    /// 绘制实心长方形
    /// </summary>
    public static GameObject DrawRectangleSolid(Transform t, float length, float width)
    {
        List<Vector3> vertices = new List<Vector3>();
        Vector3 center = t.position;
        center += new Vector3(0, 0.01f, 0);
    ...
    /// 绘制扇形区域
    /// </summary>
    public static GameObject DrawSectorSolid(Transform t, float angle, float radius)
    {
    ...
    public static void CreateMesh(List<Vector3> vertices, string name)
    {
        int[] triangles;
        Mesh mesh = new Mesh();
