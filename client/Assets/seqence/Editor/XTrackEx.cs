...
namespace UnityEditor.Seqence
{
    ...
    {
        public static List<XSeqenceObject> TrackAssets(this XTrack track)
        {
            var clips = track.clips.ToList();
            var marks = track.marks.ToList();
            var childs = track.childs.ToList();
            var list = new List<XSeqenceObject>();
            for (int i = 0; i < clips.Count; i++)
            {
                XSeqenceObject clip = clips[i] as XSeqenceObject;
        ...
        public static void BuildConf(this XSeqence timeline)
        {
